...
class VisualizationTest extends FunSuite with Checkers with MustMatchers {
  ...
  test("greatCircleDistance manual") {
    val x = Location(-87.82640732750883,-102.39289236101074)
    greatCircleDistance(x, x) must be <= 1.0
  }

  test("greatCircleDistance manual 2") {
    val x = Location(50.0359, 5.4253)
    val y = Location(58.3838, 3.0412)
    val expDist = 940900
  ...
  test("greatCircleDistance") {
    check((x: Location) => greatCircleDistance(x, x) <= 0.2)
  }

  test("predict temperature manual") {
    val x = Location(18.0,-176.0)
    val y = Location(88.0,-176.0)
    val z = Location(88.0,-176.0)
    tempDistProp(x, y, z) mustBe true
  }

  test("predict temperature") {
    check((x: Location, y: Location, z: Location) => tempDistProp(x, y, z))
  }

  private def tempDistProp (x: Location, y: Location, z: Location) = {
    val tempX = 10.0
    val tempY = 20.0
    val temperatures = List((x, tempX), (y, tempY))
    val tempZ = predictTemperature(temperatures, z)
